# Data pre-processing for the temporal scheduling and preference project
...
# Restructure data
pilot_raw <- read_csv("./data/raw/pilot.csv") %>%
  mutate(
    id = as.factor(id),
...
# Inspect exclusions
cat("--- PILOT PARTICIPANT COUNTS ---\n")

pilot_counts <- pilot_raw %>%
  count(is_included)

pilot_counts

# Filter data
pilot_prepared <- pilot_raw %>%
  filter(is_included == "Yes") %>%
  select(-is_included)

saveRDS(pilot_prepared, "./data/prepared/pilot.Rds")

# Experiment 1 ----

# Read raw
exp1_raw <- read_csv("./data/raw/exp1.csv")

# Inspect exclusions
cat("--- EXP 1 PARTICIPANT COUNTS ---\n")
n_total <- nrow(exp1_raw)
n_incl  <- nrow(drop_na(exp1_raw))
cat("Total rows raw:   ", n_total, "\n")
cat("Rows dropped (NA):", n_total - n_incl, "\n")
cat("Final Included:   ", n_incl, "\n")
...
exp2_restructured <- read_csv("./data/raw/exp2.csv") %>%
  ...
  ungroup() %>%
  select(
    id, Finished, gender, age, attention_check, focus, serious,
    pref_atts, pref_tasks, which_now, which_later, 
    trial, condition, delay, difficulty, attractiveness, choice
  )

# Inspect exclusions
cat("--- EXP 2 PARTICIPANT COUNTS ---\n")
exp2_participants <- exp2_restructured %>%
  ungroup() %>% 
  distinct(id, attention_check)

n_recruited <- nrow(exp2_participants)
n_attn_pass <- sum(exp2_participants$attention_check == 1)
n_excluded  <- n_recruited - n_attn_pass

cat("Recruited (Total):", n_recruited, "\n")
cat("Excluded (Attn Fail):", n_excluded, "\n")
cat("Included (Final):", n_attn_pass, "\n")
# Check: 386 Recruited - 126 Excluded = 260 Included?

# Filter and transform
exp2_prepared <- exp2_restructured %>%
  # filter(Finished == "True") %>%
  filter(attention_check == 1) %>%
  select(-attention_check, -Finished) %>%
  drop_na() %>%
...
saveRDS(exp2_prepared, "./data/prepared/exp2.Rds")
...
# Restructure data
exp3_raw <- read_csv("./data/raw/exp3.csv") %>%
  slice(3:n()) %>%
  mutate(
  ...
  select(
    id, Finished, attn_check, gender, age,
    pref_atts, pref_tasks, which_now, which_later,
...
# Inspect exclusions
cat("--- EXP 3 PARTICIPANT COUNTS ---\n")
exp3_participants <- exp3_raw %>%
  ungroup() %>%
  distinct(id, Finished, attn_check)

n_recruited <- nrow(exp3_participants)
n_finished  <- sum(exp3_participants$Finished == "True")
n_attn_pass <- sum(exp3_participants$Finished == "True" & exp3_participants$attn_check == 1)
n_excluded  <- n_recruited - n_attn_pass

cat("Recruited (Total):", n_recruited, "\n")
cat("Excluded (Unfinished + Attn Fail):", n_excluded, "\n")
cat("Included (Final):", n_attn_pass, "\n")
# Check: 495 Recruited - 189 Excluded = 306 Included?

# Filter and transform
exp3_prepared <- exp3_raw %>%
  filter(Finished == "True") %>%
  filter(attn_check == 1) %>%
  select(-attn_check, -Finished) %>%
  drop_na() %>%
  mutate(
